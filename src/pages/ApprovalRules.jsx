        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {rules.map(r => (
            <Card key={r.id} className="p-5">
              ...
              <div className="mt-2 space-y-1 text-xs text-muted-foreground"><p>Stage {r.approval_order}: {CHURCH_ROLES[r.required_role] || r.required_role}</p><p>Type: <span className="capitalize">{r.transaction_type}</span></p><p>Amount: {r.min_amount || 0}{r.max_amount ? ` – ${new Intl.NumberFormat().format(r.max_amount)}` : '+'}</p><p>Branch: {branchName(r.branch_id)}</p></div>

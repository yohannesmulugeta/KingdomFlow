...
export default function Income() {
  ...
  const canCreate = can('can_create_income');
  ...
  const [form, setForm] = useState({ amount: '', date: new Date().toISOString().split('T')[0], description: '', branch_id: userBranchId || '', category_id: '', category_name: '', fund_id: '', payment_method: 'cash', donor_name: '', is_anonymous_donor: false, receipt_needed: false, notes: '', status: 'draft' });
  ...
  const load = () => {
    base44.functions.invoke('getReports', {
      type: 'income', branch_id: accessScope === 'assigned_branch' ? userBranchId : undefined,
      page, pageSize: perPage, status: ['draft','pending','approved','paid','completed','rejected'],
    }).then(res => {
      setTransactions(res.data?.transactions || []);
      setLoading(false);
    }).catch(() => {
      base44.entities.Transaction.filter({ type: 'income', status: { $in: ['draft','pending','approved','paid','completed','rejected'] } }, '-date', perPage, (page-1)*perPage).then(setTransactions).finally(() => setLoading(false));
    });
    Promise.all([base44.entities.Branch.list(), base44.entities.IncomeCategory.filter({ is_active: true }), base44.entities.Fund.filter({ is_active: true })]).then(([b, c, f]) => { setBranches(b); setCategories(c); setFunds(f); });
  };
  ...
  const handleSave = async (status) => {
    ...
    try {
      const res = await base44.functions.invoke('createTransaction', {
        ...form, amount: parseFloat(form.amount), type: 'income', status,
        branch_id: form.branch_id || userBranchId,
      });
      if (res.data?.error) { toast({ title: 'Error', description: res.data.error, variant: 'destructive' }); }
      else {
        toast({ title: status === 'draft' ? 'Draft saved' : status === 'pending' ? 'Submitted for approval' : 'Created' });
  ...
  const handleVoid = async () => {
    ...
    toast({ title: 'Transaction voided' }); setVoidOpen(null); setVoidReason(''); load();
  };
  ...
  return (
    <div>
      <PageHeader title="Income" description="Record and manage income transactions">
        {canCreate && <Button onClick={() => setFormOpen(true)}><PlusCircle className="w-4 h-4 mr-2" /> Record Income</Button>}
      </PageHeader>

      {transactions.length === 0 ? (
        <EmptyState icon={PlusCircle} title="No income recorded" description="Record your first income transaction" actionLabel={canCreate ? "Record Income" : undefined} onAction={canCreate ? () => setFormOpen(true) : undefined} />
      ) : (
        <>
          <Card><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/50"><th className="px-4 py-3 text-left">Number</th><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-left">Donor</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3 text-left">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>{transactions.map(t => (<tr key={t.id} className="border-b last:border-0 hover:bg-muted/30"><td className="px-4 py-3 font-medium text-xs">{t.transaction_number}</td><td className="px-4 py-3 text-muted-foreground">{t.date}</td><td className="px-4 py-3">{t.description || t.category_name || '—'}</td><td className="px-4 py-3 text-muted-foreground">{t.donor_name || (t.is_anonymous_donor ? 'Anonymous' : '—')}</td><td className="px-4 py-3 text-right font-medium text-emerald-600">+{fm(t.amount)}</td><td className="px-4 py-3"><span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${STATUS_COLORS[t.status] || 'bg-gray-100'}`}>{t.status}</span></td><td className="px-4 py-3 text-right">{t.status === 'draft' && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => handleSubmit(t.id)}>Submit</Button>}{['approved','paid','completed'].includes(t.status) && <Button variant="ghost" size="sm" className="h-7 text-xs text-destructive" onClick={() => { setVoidOpen(t); setVoidReason(''); }}><Ban className="w-3 h-3 mr-1" /> Void</Button>}</td></tr>))}</tbody></table></div></Card>

...
export default function Income() {
  ...
  const load = () => {
    base44.entities.Transaction.filter({ type: 'income' }, '-date', 50).then(data => {
  ...
  return (
    <div>
      <PageHeader title="Income" description="Record and manage income transactions">
        <Button onClick={() => { setEditing(null); setFormOpen(true); }}><Plus className="w-4 h-4 mr-2" /> Record Income</Button>
      </PageHeader>

      {transactions.length === 0 ? (
        <EmptyState icon={ArrowDownCircle} title="No income recorded" description="Start by recording your first income transaction" actionLabel="Record Income" onAction={() => { setEditing(null); setFormOpen(true); }} />
      ) : (
        <Card>
              ...
              <tbody>
                {transactions.map(t => (
                  <tr key={t.id} className="border-b last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-right font-medium text-emerald-700">{fmt(t.amount)}</td>
      ...
      <TransactionForm open={formOpen} onOpenChange={setFormOpen} type="income" editing={editing} onSaved={load} />
    </div>
  );
}